...
*** 2. Sub-seeds
    Add /index/ (integer between 0 and 3^36, maximum 12 trytes) to /seed/, hash it once and generate the /*sub-seed*/.
...
*** 3. Private key
    Absort /subseed's trits/ into the sponge function then spit out the /*private key*/ of length proportional with the /security level/.
...
*** Note
   ...
   You also need /*conv*/ package from /pyota/ library, implementation to convert trits to/from bytes is a bit more complex due to endianess / signed bytes encoding and will make the blog post too long and hey, it's holiday time.
